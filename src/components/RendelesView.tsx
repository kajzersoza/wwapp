          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0">
            ...
            {[
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors cursor-pointer whitespace-nowrap inline-flex items-center gap-1.5 ${
                  statusFilter === st.id
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-3 border-t border-stone-100 text-xs">
          <div className="flex items-center gap-2">
            ...
            <div className="inline-flex rounded-lg border border-stone-200 bg-stone-50 p-0.5">
              ...
              >
                Kapcsolatokkal rendelkező tételek
              </button>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button
              type="button"
              onClick={expandAll}
              className="text-xs text-[#006067] hover:underline font-semibold flex items-center gap-1"
            >
              <ChevronDown className="w-3.5 h-3.5" />
              <span>Összes kapcsolat kinyitása</span>
            </button>
            <span className="text-stone-300">|</span>
            <button
              type="button"
              onClick={collapseAll}
              className="text-xs text-stone-600 hover:underline font-semibold flex items-center gap-1"
            >
              <ChevronUp className="w-3.5 h-3.5" />
              <span>Összes becsukása</span>
            </button>
          </div>
        </div>
      </div>
          ...
          <div className="p-12 text-center text-stone-500 space-y-3">
            ...
            {(searchQuery || statusFilter !== 'all') && (
              <button
                ...
                className="inline-flex items-center gap-1 text-xs text-[#006067] font-bold hover:underline"
              >
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="bg-stone-50/90 border-b border-stone-200 text-[11px] font-bold uppercase tracking-wider text-stone-600">
                  ...
                  <th className="py-3 px-3 font-mono">Termék ID</th>
                  <th className="py-3 px-3">Terméknév</th>
                  <th className="py-3 px-3">Státusz</th>
                  <th className="py-3 px-3">Dátum</th>
                  <th className="py-3 px-3">Dátum Megrendelve</th>
                  <th className="py-3 px-3">Dátum Raktárban</th>
                  <th className="py-3 px-2 w-20 text-center">Kapcsolódó</th>
                      ...
                      >
                        ...
                        <td className="py-3 px-3 whitespace-nowrap">
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded bg-stone-100 hover:bg-[#006067] hover:text-white font-mono font-bold text-stone-900 border border-stone-200 transition-all cursor-pointer group"
                            title={`Kattintson ide a(z) ${order.termekId} termék adatlapjának megnyitásához!`}
                          >
                            <span>{order.termekId}</span>
                            <ExternalLink className="w-3 h-3 text-stone-400 group-hover:text-white opacity-0 group-hover:opacity-100 transition-opacity" />
                          </button>
                        </td>

                        {/* Terméknév (csak a név, nem kell kategória és darabszám) */}
                        <td className="py-3 px-3 max-w-sm">
                          <button
                            type="button"
                            onClick={() => selectProductById(order.termekId)}
                            className="text-left font-semibold text-stone-900 hover:text-[#006067] transition-colors line-clamp-2 cursor-pointer"
                        ...
                        {/* 3. Státusz */}
                        <td className="py-3 px-3 whitespace-nowrap">{getStatusBadge(order.statusz)}</td>

                        {/* 4. Dátum */}
                        <td className="py-3 px-3 font-mono text-[11px] text-stone-700 whitespace-nowrap">
                          {order.datum ? (
                            <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-stone-50 border border-stone-200 text-stone-800">
                              <Calendar className="w-3 h-3 text-stone-400 flex-shrink-0" />
                              <span>{order.datum}</span>
                            </span>
                          ) : (
                            <span className="text-stone-300 pl-2">—</span>
                          )}
                        </td>

                        {/* 5. Dátum Megrendelve */}
                        <td className="py-3 px-3 font-mono text-[11px] text-stone-700 whitespace-nowrap">
                          {order.datumMegrendelve ? (
                            <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-stone-50 border border-stone-200 text-stone-800">
                              <Calendar className="w-3 h-3 text-stone-400 flex-shrink-0" />
                              <span>{order.datumMegrendelve}</span>
                            </span>
                          ) : (
                            <span className="text-stone-300 pl-2">—</span>
                          )}
                        ...
                        {/* 6. Dátum Raktárban */}
                        <td className="py-3 px-3 font-mono text-[11px] text-stone-700 whitespace-nowrap">
                          {order.datumRaktarban ? (
                            <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-stone-50 border border-stone-200 text-stone-800">
                              <Calendar className="w-3 h-3 text-stone-400 flex-shrink-0" />
                              <span>{order.datumRaktarban}</span>
                            </span>
                          ) : (
                            <span className="text-stone-300 pl-2">—</span>
                          )}
                        ...
                        {/* Kapcsolódó termékek: vékonyabb oszlop, csak a szám */}
                        <td className="py-3 px-2 text-center whitespace-nowrap">
                          {relatedProducts.length > 0 ? (
                            <button
                              type="button"
                              onClick={() => toggleExpand(order.id)}
                              className="inline-flex items-center justify-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-50 text-amber-900 border border-amber-200 hover:bg-amber-100 transition-colors cursor-pointer"
                        ...
                        <td className="py-3 px-3 text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            <button
                              ...
                              className="p-1.5 rounded-lg border border-stone-200 text-stone-500 hover:text-[#006067] hover:border-[#006067]/40 hover:bg-stone-50 transition-colors cursor-pointer"
                            ...
                            {deleteConfirmId === order.id ? (
                              ...
                              <button
                                ...
                                className="p-1.5 rounded-lg border border-stone-200 text-stone-400 hover:text-red-600 hover:border-red-200 hover:bg-red-50 transition-colors cursor-pointer"
